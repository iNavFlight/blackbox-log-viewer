; ------------------------------------------
; Installer for INAV Blackbox Explorer
; ------------------------------------------
...
#define ApplicationName "INAV Blackbox Explorer"
...
[CustomMessages]
AppName=inav-blackbox-explorer
...
[Setup]
AppId=2e5662ca-1fb3-8f1e-a7e1-e390add7a1be
...
begin
    ...
    if not RegQueryStringValue(HKLM, 'SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\INAV Blackbox Explorer', 'UninstallString', Result) then
    begin
        if not RegQueryStringValue(HKLM, 'SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\INAV Blackbox Explorer', 'UninstallString', Result) then
        begin
            RegQueryStringValue(HKCU, 'SOFTWARE\INAV\INAV Blackbox Explorer', 'UninstallString', Result)
        end;
    end;
end;
